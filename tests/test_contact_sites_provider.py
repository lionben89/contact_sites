...
images_repo = quilt3.Package.browse("aics/pipeline_integrated_cell", registry="s3://allencell")
...
def fetch_images() -> None:
    ...
    for repo_internal_path in images.keys():
        ...
        for image_obj in images_objs:
            ...
            image_repo_path = "{}/{}".format(repo_internal_path,image_file_name)

            # construct image target path
            image_target_path = "{}/{}".format(images_save_dir,image_file_name)
...
def test_provider() -> None:
    ...
    for repo_internal_path in images.keys():
        images_objs = images.get(repo_internal_path)
        for image_obj in images_objs:
            image_file_name = "{}/{}".format(images_save_dir,image_obj.get("file_name"))
    ...
    # pridiction pipe
    # result = provider.run_pipe(contact_sites_images,"ER_Golgi_Mito_predictions", image_to_ndarray=lambda image:_image_to_ndarray(image,"CYX"), unifier=False)
    ...
    image_target_path_png = "{}/result_Mem_ER_DNA_GT.png".format(results_save_dir)
    #result.save_figure(image_target_path_png,image_to_legend_ratio=1)
    image_target_path_tiff = "{}/result_Mem_ER_DNA_GT.tiff".format(results_save_dir)
    ...
    organelle_distribuation_path_fig = "{}/z_slices_organelle_distribuation.png".format(results_save_dir)
    z_dim=0
    result.save_distribuation_figure(z_dim,organelle_distribuation_path_fig)
